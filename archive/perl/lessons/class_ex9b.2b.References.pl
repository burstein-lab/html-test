...
#
# Output: all the product names and db_xref of the proteins coded 
#		  on negative strand.
###################################################################
use strict;
...
my %genBank;
my ($product, $protein_id, $db_xref, $strand);
foreach my $line (@genBanklines){
	# Find CDS lines, such as:
	# '     CDS             complement(join(3844..5180,5459..5471))'
	# and check wheather 'complement' exist
	if ($line =~ m/^\s+CDS\s+(complement)?/){
		if (defined $1) {
			$strand = '-';
		}
		else {
	...
	# '                    /product="E1A"'
	# use product as key and insert strand to data structure
	if ($line =~ m/\s+\/product=\"([^"]+)\"/){
		$product = $1;
		$genBank{$product}{"strand"} = $strand;
	}
...
foreach my $product (@products){
	if ( $genBank{$product}{"strand"} eq '-') {
		print "The product $product is coded on the negative strand\n";
	}
}
